...
# Tell robot to drive a single joint in a diraction
def drive_joint(joint, dir): 
    ...
    errorcode = "Everything is fine!"
    l_errorcode.config(text = errorcode)

# Tell robot to zero all joints
def set_home():
    command = "12\n" # command 12 = home
    #send command
    msg = messagebox.showinfo("Send Command", "%r" % command)
...
# Tell robot to set motor state to value
...
def run_program():
    msg = messagebox.showinfo("1", "run")

def pause_program():
    msg = messagebox.showinfo("1", "pause")

def stop_program():
    msg = messagebox.showinfo("1", "stop")

#send robot new parameter settings for zone
def set_params():
